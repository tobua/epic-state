...
type Config = Parameters<
  (Window extends { __REDUX_DEVTOOLS_EXTENSION__?: infer T } ? T : any)['connect']
>[0]
 ...
 */
export function devtools<T extends object>(proxyObject: T, name?: string): (() => void) | undefined
 ...
 */
export function devtools<T extends object>(proxyObject: T, options?: Options | string) {
  ...
  try {
    extension =
      (enabled ?? process.env.NODE_ENV !== 'production') && window.__REDUX_DEVTOOLS_EXTENSION__
  } catch {
    // ignored
  }
  if (!extension) {
